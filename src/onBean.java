...
public Hashtable getcustfortut(String id)
	{
		Hashtable returnvalue=new Hashtable();
		String query="select a.CUST_ID,a.cust_name,a.cust_addr1,a.CUST_MOBILE,b.tutorid from customer a,batch b where a.BATCHID=b.BATCHID and b.BATCHID='"+id+"'";
		System.out.println(query);
		try{
		Statement st1=con.createStatement();
		ResultSet rs=st1.executeQuery(query);
...
public Hashtable getCustomerDetails(String id)
	{
		Hashtable returnvalue=new Hashtable();
		String query="select CUST_ID,CUST_NAME,CUST_ADDR1,CUST_CITY,CUST_STATE,CUST_COUNTRY,CUST_FAX,TO_CHAR(CUST_DOB,'DD-MM-YYYY') DOB,CUST_MAIL,SEX,CUST_RESNO,CUST_OFFNO,CUST_MOBILE,CUST_ADD2,BATCHID from customer where cust_id='"+id+"'";
		System.out.println(query);
		try{
		Statement st1=con.createStatement();
		ResultSet rs=st1.executeQuery(query);
