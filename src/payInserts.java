...
public class payInserts extends HttpServlet {
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (selec.equals("ByD.D")) {
			...
			String qr2 = "insert into dddetails values (" + c_no + ",'" + p_date + "','" + c_bank + "','" + camount
					+ "'," + pid + ")";
			...
			if (result.equals("true")) {
				...
				String qr = "insert into paymentMaster values(" + pid + ",'" + camount + "',sysdate(),'bycreditcard','"
						+ cidd + "')";
				String qu = "update batch set STRENGTH=STRENGTH+1 where BATCH_ID='" + bids + "'";
				obj.setCommitState(false);
				int i = obj.insertData(qr);
